...
# Hard ceiling on a single pipeline turn (ASR + agent loop + TTS).  A
# stuck LLM call shouldn't pin the FSM forever — past this we bail and
# return control to the client.  3 minutes is generous (typical turns
# are 1-4 s; vision-loop fallback worst case is ~60 s).
PIPELINE_TIMEOUT_S = 180.0

# WebSocket keepalive ping interval.  Tuned to fire well under common
# idle-connection cutoffs: most reverse-proxies / load-balancers drop
# WS at 30-60 s of silence; 25 s gives one safety beat.
WS_KEEPALIVE_INTERVAL_S = 25.0


# Session registry + voicemail fan-out moved to ws_registry.py — that
# module is import-cheap (no aiortc / VAD pull-in) so pipeline.py can
# notify recipients without dragging the WS stack along.  Re-export the
# public names so existing call sites in this file keep working.
from .ws_registry import (
    _SESSIONS_BY_PROFILE,
    notify_voicemail_arrived,
    register_session_profile as _register_session_profile,
    unregister_session_profile as _unregister_session_profile,
)
...
async def _keepalive(ws: WebSocket, interval_s: float = WS_KEEPALIVE_INTERVAL_S) -> None:
    try:
        while True:
